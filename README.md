...
### 0. بناء الصورة تلقائيًا (GitHub Actions)

[`.github/workflows/ci.yml`](.github/workflows/ci.yml) يعمل على كل PR وكل push:

1. **quality** — `lint` + `typecheck` + `test` (بمتغيرات بيئة وهمية، بلا خدمات حقيقية).
2. **image** — بعد نجاح الأولى فقط: يبني الصورة لـ `linux/amd64`. في الـ PR يبني فقط للتحقق؛ على `main` ووسوم `v*.*.*` يرفعها إلى `ghcr.io/amdlre/seera`.

| الوسم           | متى                                     | الاستخدام                              |
| --------------- | --------------------------------------- | -------------------------------------- |
| `latest`        | كل push على `main`                      | آخر نسخة                               |
| `sha-xxxxxxx`   | كل push                                 | نسخة ثابتة — للنشر والرجوع لنسخة سابقة |
| `1.2.3` / `1.2` | عند `git tag v1.2.3 && git push --tags` | الإصدارات الرسمية                      |
...
1. **CranL → Applications → New Database** → PostgreSQL، المنطقة نفسها للتطبيق (Saudi Arabia). التطبيق يستخدم الاتصال **Internal** (القاعدة غير معرَّضة للإنترنت)، والهجرات تُطبَّق تلقائيًا عند إقلاع الحاوية.
...
ADMIN_EMAILS=you@example.com                         # اختياري — بريد (أو أكثر مفصولة بفاصلة) يملك صلاحية الأدمن
```
...
`ADMIN_EMAILS` هو مصدر الحقيقة: أي بريد فيه يُنشأ حسابه بدور `admin` من أول تسجيل دخول، ويُعاد إليه الدور في كل دخول لاحق إن غُيِّر. المقارنة غير حسّاسة لحالة الأحرف. حذف بريد من القائمة **لا** يسحب الدور تلقائيًا — يُسحب من لوحة الإدارة، حتى لا يفقد فريق كامل صلاحياته بخطأ في متغيّر.

### 4. الهجرات (تلقائية)

لا خطوة يدوية: `CMD` في `Dockerfile` يشغّل `node migrate.cjs` قبل الخادم عند كل إقلاع للحاوية. السكربت ([`src/db/migrate.ts`](src/db/migrate.ts)) يُحزَم في ملف واحد أثناء البناء (`npm run build:migrate`) فلا تحتاج صورة الإنتاج إلى drizzle-kit ولا إلى مصادر TypeScript، ويعمل عبر الشبكة الداخلية لـ CranL بلا تعريض القاعدة للإنترنت.

- **قفل استشاري (advisory lock)** يمنع حاويتين تقلعان معًا من تطبيق الهجرة مرتين.
- **فشل الهجرة يوقف الإقلاع** بدل تشغيل الخادم على مخطط ناقص.
- محليًا يبقى `npm run db:migrate` (drizzle-kit) كما هو.
